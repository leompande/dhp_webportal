(function() {
    ...
    angular
        .module('dhp',['openlayers-directive','datatables'])
    ...
    function mainController($http) {
        ...
        main.getAppropiatePdf= function(orunitUid,period){
            var base = "https://139.162.204.124/mvc/";
            $.post( base + "dhis-web-commons-security/login.action?authOnly=true", {
                j_username: "portal", j_password: "Portal123"
            },function(){
                var url = "";
                if($scope.selectedOrgUnit == "m0frOspS7JY"){
                    url = "https://139.162.204.124/mvc/api/analytics.csv?dimension=dx:"+id+"&dimension=pe:"+$scope.selectedPeriod+"&dimension=ou:LEVEL-1;LEVEL-2;"+$scope.selectedOrgUnit+"&displayProperty=NAME&tableLayout=true&columns=dx&rows=pe;ou";
                }else{

                    url = "https://139.162.204.124/mvc/api/analytics.csv?dimension=dx:"+id+"&dimension=pe:"+$scope.selectedPeriod+"&dimension=ou:LEVEL-2;LEVEL-3;"+$scope.selectedOrgUnit+"&displayProperty=NAME&tableLayout=true&columns=dx&rows=pe;ou";
                }
        ...
        main.getHtmlReport= function(orunitUid,period){
            ...
            $.post( base + "dhis-web-commons-security/login.action?authOnly=true", {
                j_username: "portal", j_password: "Portal123"
                    ...
                    var url = "https://apps.dhis2.org/demo/api/dataValueSets.json?dataSet=pBOMPrpg1QX&period="+period+"&orgUnit="+orunitUid;

                $http.get(url,{'Content-Type': 'application/json'}).success(function(data){
                    console.log(data);
